module RoomsHelper
  ...
  def most_new_message_preview(room)
    ...
    if message.present?
      # メッセージがあれば内容を表示
      tag.p message.content.to_s, class: 'dm_list__content_time__link__box__message'
    else
  ...
  def most_new_message_time(room)
    ...
    if message.present?
      # メッセージがあれば内容を表示
      tag.p message.created_at.strftime('%Y/%m/%d %H:%M').to_s, class: 'dm_list__content__link__box__message'
    else
      # メッセージがなければ[ まだメッセージはありません ]を表示
      tag.p '[ まだメッセージはありません ]', class: 'dm_list__content__link__box__message'
    end
  ...
  def image_user(room)
    # 中間テーブルから相手ユーザーのデータを取得
    entry = room.entries.where.not(user_id: current_user)
    image = entry.first&.user.present? ? entry.first.user.image : 'no_user.png'
    # 相手ユーザーの画像を表示
    if entry.first&.user&.sex == '男性'
      image_tag image.to_s, class: 'icon_image_men_dm'
    elsif entry.first&.user&.sex == '女性'
      image_tag image.to_s, class: 'icon_image_women_dm'
    elsif entry.first&.user&.sex == 'その他'
      image_tag image.to_s, class: 'icon_image_other_dm'
    else
      image_tag image.to_s, class: 'icon_image_remove_dm'
    end
  end
end
